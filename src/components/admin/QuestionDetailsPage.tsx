...
import { fetchAnswersByQuestionId, fetchAllQuestionsAdmin } from "../api/adminSurveyApi";
...
const QuestionDetailPage: React.FC = () => {
  ...
  const answerStats: AnswerStats = useMemo(() => {
    ...
    const longest = answerTexts.reduce((longest, current) => 
      current.length > longest.length ? current : longest, "");
    
    const shortest = answerTexts.reduce((shortest, current) => 
      current.length < shortest.length ? current : shortest, answerTexts[0] || "");
  ...
  const chartData = {
    labels: filteredAnswers.slice(0, 10).map(([answer]) => 
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: { 
        display: true, 
        text: `${question?.question.substring(0, 50)}${question?.question && question.question.length > 50 ? '...' : ''}` 
      },
      tooltip: {
        callbacks: {
          label: function(context: any) {
            const fullAnswer = filteredAnswers[context.dataIndex]?.[0] || '';
            return `${fullAnswer}: ${context.parsed.y || context.parsed.x} responses`;
          }
        }
      }
    },
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="text-purple-700 text-lg font-medium">Loading question details...</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="p-6">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
            <div className="text-red-600 mb-4">
              ...
              <p className="text-lg font-semibold">Error loading question details</p>
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50">
        <div className="p-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
            ...
            <h3 className="text-xl font-semibold text-blue-900 mb-2">No Responses Found</h3>
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50">
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold text-gray-900">Question Analysis</h2>
            <p className="text-gray-600 mt-1">Detailed response analysis and statistics</p>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                question.questionLevel === 'Beginner' ? 'bg-green-100 text-green-800' :
                question.questionLevel === 'Intermediate' ? 'bg-yellow-100 text-yellow-800' :
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="font-mono text-xs text-gray-700">{question.questionID}</p>
              </div>
              <div>
                <span className="text-gray-500">Type:</span>
                <p className="font-medium">{question.questionType === 'Mcq' ? 'Multiple Choice' : 'Text Input'}</p>
              </div>
              <div>
                <span className="text-gray-500">Total Responses:</span>
                <p className="font-bold text-purple-600">{answerStats.totalAnswers}</p>
              </div>
              <div>
                <span className="text-gray-500">Unique Answers:</span>
                <p className="font-bold text-blue-600">{answerStats.uniqueAnswers}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {viewMode === 'list' && (
          <div className="bg-white rounded-xl shadow-lg border border-gray-100">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-lg font-bold">All Responses ({filteredAnswers.length})</h3>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-purple-600">{answerStats.totalAnswers}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Unique Answers:</span>
                  <span className="font-bold text-blue-600">{answerStats.uniqueAnswers}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Average Length:</span>
                  <span className="font-bold text-green-600">{answerStats.averageLength} chars</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Response Rate:</span>
                  <span className="font-bold text-orange-600">
                    {answerStats.uniqueAnswers > 0 ? 
                      ((answerStats.uniqueAnswers / answerStats.totalAnswers) * 100).toFixed(1) + '%' : 
                      '0%'
                    } unique
                  </span>
                </div>
              ...
              {answerStats.mostCommonAnswer ? (
                <div className="space-y-3">
                  <div className="p-4 bg-purple-50 rounded-lg border-l-4 border-purple-500">
                    <p className="text-gray-900 font-medium">{answerStats.mostCommonAnswer}</p>
                  </div>
                  <p className="text-sm text-gray-600">
                    Given {answerFrequencies[answerStats.mostCommonAnswer]} times 
                    ({((answerFrequencies[answerStats.mostCommonAnswer] / answerStats.totalAnswers) * 100).toFixed(1)}% of responses)
                  </p>
                </div>
              ) : (
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <span className="text-sm text-gray-600">Shortest Answer:</span>
                  <p className="text-sm bg-gray-50 p-2 rounded mt-1 break-words">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
              ...
              <div className="h-64">
                <Pie 
                  data={{
                    labels: filteredAnswers.slice(0, 5).map(([answer]) => 
                      answer.length > 20 ? answer.substring(0, 20) + '...' : answer
                    ),
                    datasets: [{
                      data: filteredAnswers.slice(0, 5).map(([, freq]) => freq),
                      backgroundColor: [
                        '#8B5CF6', '#06B6D4', '#10B981', '#F59E0B', '#EF4444'
                      ],
                    }],
